...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus required_status = DocumentStatus::ACTUAL) const {
        return FindTopDocuments(raw_query,
                                [required_status](int document_id, DocumentStatus status, int rating)
                                { return status == required_status; });
    }
    
    template <typename FilterFunc> 
    vector<Document> FindTopDocuments(const string& raw_query, FilterFunc filter_func) const {
    ...
    vector<Document> FindAllDocuments(const Query& query, FilterFunc filter_func) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                if (filter_func(document_id, documents_.at(document_id).status, documents_.at(document_id).rating)){
