    ...
    explicit SearchServer(const StringContainer& stop_words)
        : stop_words_(MakeUniqueNonEmptyStrings(stop_words)) {
         if(!all_of(stop_words_.begin(), stop_words_.end(),
          [](std::string word) { return IsValidWord(word); })){
            throw std::invalid_argument("Incorrect document text"s);
          }
    }
    ...
    std::vector<Document> FindTopDocuments(const std::string& raw_query,
        ...
        sort(matched_documents.begin(), matched_documents.end(),
             [](const Document& lhs, const Document& rhs) {
                 if (std::abs(lhs.relevance - rhs.relevance) < MIN_DIFFERENCE_BETWEEN_RELEVANCE) {
